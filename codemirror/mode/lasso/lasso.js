(function() {
    ...
    hooks: {
      ...
      "/": function(stream, state) {
        if (stream.eat("*")) {
          ...
          while (!stream.eol() && !stream.match("?>", false)) stream.skipToEnd();
          return "comment";        
        }
        return false;
      }
  ...
  CodeMirror.defineMode("lasso", function(config, parserConfig) {
    var htmlMode = CodeMirror.getMode(config, {name: "xml", htmlMode: true});
    var jsMode = CodeMirror.getMode(config, "javascript");
    var cssMode = CodeMirror.getMode(config, "css");
    var lassoMode = CodeMirror.getMode(config, lassoConfig);

    function dispatch(stream, state) { // TODO open PHP inside text/css
      var isLasso = state.mode == "lasso";
